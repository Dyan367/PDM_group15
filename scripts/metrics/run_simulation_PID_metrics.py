...
# ======== Modified: Implement point_to_line_distance Function ========
...
if __name__ == "__main__":
    ...
    print(f"Optimality: {optimality:.2f}%")  # Added formatting for percentage
    ...
    for step in range(total_steps):
        ...
        if distance_to_waypoint < waypoint_threshold:
            print(f"Reached waypoint {waypoint_idx} at step {step}, time {step * mpc_params['dt']:.2f}s with distance {distance_to_waypoint:.4f} m")
    ...
    time_axis = np.arange(len(distance_errors)) * mpc_params['dt']
    plt.plot(time_axis, distance_errors, label='Distance to Closest Path Segment')
    plt.xlabel('Time (s)')
