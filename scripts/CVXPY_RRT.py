...
# Define the CVXPY-based Drone Controller class
class DroneControllerCVXPY:
    def __init__(self, mass, gravity=9.81, max_thrust=10, max_torque=2.0, Kp_pos=1.0, Kp_vel=1.0, Kp_rpy=1.0):
        self.mass = mass
    ...
    def computeControl(self,
        ...
        # Define variables for thrust and torques
        thrust = cp.Variable()  # Total thrust
        tau_x = cp.Variable()  # Torque in x-direction (roll)
        tau_y = cp.Variable()  # Torque in y-direction (pitch)
        tau_z = cp.Variable()  # Torque in z-direction (yaw)

        # Step 1: Define the position, velocity, and orientation errors
        pos_error = target_pos - cur_pos
        vel_error = target_vel - cur_vel

        # Using roll, pitch, yaw from current quaternion (convert quaternion to RPY)
        current_roll, current_pitch, current_yaw = self.quat_to_rpy(cur_quat)
        rpy_error = target_rpy - np.array([current_roll, current_pitch, current_yaw])
        rpy_rate_error = target_rpy_rates - cur_ang_vel

        # Step 2: Define the cost function
        position_cost = self.Kp_pos * cp.norm(pos_error, 2)
        velocity_cost = self.Kp_vel * cp.norm(vel_error, 2)
        orientation_cost = self.Kp_rpy * cp.norm(rpy_error, 2)
        control_cost = cp.norm(thrust, 1) + cp.norm(cp.vstack([tau_x, tau_y, tau_z]), 1)  # Penalize thrust and torques

        total_cost = position_cost + velocity_cost + orientation_cost + control_cost

        # Step 3: Define the constraints
        constraints = [
            thrust >= 0,  # Thrust cannot be negative
            thrust <= self.max_thrust,  # Maximum thrust
            tau_x >= -self.max_torque,  # Maximum torque (roll)
            tau_x <= self.max_torque,
            tau_y >= -self.max_torque,  # Maximum torque (pitch)
            tau_y <= self.max_torque,
            tau_z >= -self.max_torque,  # Maximum torque (yaw)
            tau_z <= self.max_torque
        ]
        ...
        # Step 5: Return the optimal control inputs
        if problem.status == cp.OPTIMAL:
            print(f"Thrust: {thrust.value}, Tau_x: {tau_x.value}, Tau_y: {tau_y.value}, Tau_z: {tau_z.value}")

            return thrust.value, tau_x.value, tau_y.value, tau_z.value

        else:
            raise ValueError("Optimization problem did not converge to a solution.")
...
def main():
    ...
    controller = DroneControllerCVXPY(mass=0.027000, max_thrust=10000, max_torque=10000.0)
    ...
    for i in range(num_steps):
        ...
        if waypoint_idx < len(waypoints):
            ...
            thrust, tau_x, tau_y, tau_z = controller.computeControl(
            ...
            action[0, :] = np.array([thrust, tau_x, tau_y, tau_z])
        else:
